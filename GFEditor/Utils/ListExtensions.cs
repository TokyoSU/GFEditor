namespace GFEditor.Utils
{
    public static class ListExtensions
    {
        ...
        /// <summary>
        /// Get int value from a string list based on index position with a string removed beforehand.
        /// </summary>
        /// <param name="list">The list of strings.</param>
        /// <param name="index">A valid index, no check is being done except int.TryParse</param>
        /// <param name="valueToRemove">Remove a specific string before converting</param>
        /// <returns>Valid or 0</returns>
        public static int GetInt(this List<string> list, int index, string valueToRemove)
        {
            string result = list[index];

            // if both is not empty, then remove the value.
            if (!string.IsNullOrEmpty(result) && !string.IsNullOrEmpty(valueToRemove))
                result = result.RemoveString(valueToRemove);

            // Try parsing as a double with invariant culture
            if (double.TryParse(result, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out var dResult))
            {
                // If double is parsed, convert to int (rounding the value)
                return Convert.ToInt32(dResult);
            }
